        ...
        return (
            <div className="MonsterTracker--Container">
                <h5 className="MonsterTracker--Name">
                    {name}
                    <button onClick={() => removeMonster()}>X</button>
                </h5>
                <div className="MonsterTracker">
                    ...
                    <div className="MonsterTracker--Controls">
                        <div className="MonsterTracker--StatusEffects--ToggleAll">
                            ...
                            <StatusEffectTracker
                                ...
                                onToggle={s => toggleAllStatusEffects(s)}
                            />
                        </div>
                            ...
                            {monsters.map(({ alive, elite }, i) => (
                                <button
                                    ...
                                    className={classNames({
                                        ...
                                        'MonsterTracker--MonsterSelector--Narrow':
                                            i + 1 >= 10
                                    })}
                    ...
                    {monsters.map(({ alive, elite, level }, i) => {
                        const individualMonsterStats =
                            level !== scenarioLevel &&
                            MONSTERS[name].stats[level];
                        return (
                            alive && (
                                <React.Fragment key={i}>
                                    {individualMonsterStats && (
                                        <div className="MonsterTracker--StatsContainer">
                                            {elite ? (
                                                <MonsterStats
                                                    stats={
                                                        individualMonsterStats.elite
                                                    }
                                                    elite
                                                />
                                            ) : (
                                    ...
                                    <div
                                        className={classNames({
                                            'MonsterTracker--Monster': true,
                                            'MonsterTracker--Monster--Elite': elite
                                        })}
                                    >
                                        <div className="MonsterTracker--Monster--Controls">
                                            <div>
                                                <span className="MonsterTracker--Monster--Number">{`${i +
                                                    1}`}</span>
                                                <button
                                                    onClick={() =>
                                                        this.onToggleAlive(i)
                                                    }
                                                >
                                                    Kill
                                                </button>
...
export const MonsterTracker = connect(
    (state, ownProps) => {
        return {
            ...
            allStatusEffects: monstersSelectors.allStatusEffects(
                state,
                ownProps.name
            ),
    ...
    (dispatch, ownProps) => {
        return {
            ...
            dealDamage: (i, damage) =>
                dealDamageAction(dispatch, ownProps.name, i, damage)
        };
    }
